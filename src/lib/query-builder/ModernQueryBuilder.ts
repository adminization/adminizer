...
import { FilterCustomFieldHandler } from '../filters/FilterCustomFieldHandler';
 ...
 * - Support for FilterCustomFieldHandler
...
export class ModernQueryBuilder {
    ...
    constructor(
        ...
        private filterCustomFieldHandler?: FilterCustomFieldHandler
    ) {
    ...
    async execute(params: QueryParams): Promise<QueryResult> {
        ...
        const orderForRaw = params.sort ? [[params.sort, params.sortDirection || 'DESC']] : [['createdAt', 'DESC']];
    ...
    private buildSingleCondition(cond: FilterCondition): Record<string, any> {
        ...
        if ((fieldType === 'json' || fieldConfigType === 'select-many') &&
            ...
            if (operator === 'eq') {
                ...
                const sql = `${field} = '${jsonStr.replace(/'/g, "''")}'`;
    ...
    private handleCustomCondition(cond: FilterCondition): Record<string, any> {
        const handler = this.filterCustomFieldHandler?.get(cond.customHandler || '');
