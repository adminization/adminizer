// Lightweight i18n for the assistant panel. The active locale comes from the
// backend (agent `/status` → locale, resolved from the admin user) and the
// dictionaries are bundled, because the panel is a standalone module bundle and
// cannot reach the panel translations. English strings double as keys, so a
// missing translation falls back to readable English.
...
const ru: Dictionary = {
  ...
  'Read the documentation article "{title}" (id: {id}) and use it to answer.':
    'Прочитай статью документации «{title}» (id: {id}) и используй её для ответа.',
  'Open in knowledge base': 'Открыть в базе знаний',
  'Remove from context': 'Убрать из контекста',
  'How can I help you today?': 'Чем могу помочь?',
  'Ask about the data available to your account.': 'Спрашивайте о данных, доступных вашей учётной записи.',
  'Ask about your data… type / for commands': 'Спросите о данных… введите / для команд',
...
const de: Dictionary = {
  ...
  'Read the documentation article "{title}" (id: {id}) and use it to answer.':
    'Lies den Dokumentationsartikel „{title}“ (id: {id}) und nutze ihn für die Antwort.',
  'Open in knowledge base': 'In der Wissensdatenbank öffnen',
  'Remove from context': 'Aus dem Kontext entfernen',
  'How can I help you today?': 'Wie kann ich Ihnen helfen?',
  'Ask about the data available to your account.': 'Fragen Sie nach den Daten, die Ihrem Konto zur Verfügung stehen.',
  'Ask about your data… type / for commands': 'Nach Daten fragen… / für Befehle eingeben',
...
const es: Dictionary = {
  ...
  'Read the documentation article "{title}" (id: {id}) and use it to answer.':
    'Lee el artículo de documentación «{title}» (id: {id}) y úsalo para responder.',
  'Open in knowledge base': 'Abrir en la base de conocimiento',
  'Remove from context': 'Quitar del contexto',
  'How can I help you today?': '¿En qué puedo ayudarte hoy?',
  'Ask about the data available to your account.': 'Pregunta por los datos disponibles para tu cuenta.',
  'Ask about your data… type / for commands': 'Pregunta por tus datos… escribe / para comandos',
...
const fr: Dictionary = {
  ...
  'Read the documentation article "{title}" (id: {id}) and use it to answer.':
    'Lis l’article de documentation « {title} » (id : {id}) et utilise-le pour répondre.',
  'Open in knowledge base': 'Ouvrir dans la base de connaissances',
  'Remove from context': 'Retirer du contexte',
  'How can I help you today?': 'Comment puis-je vous aider ?',
  'Ask about the data available to your account.': 'Interrogez les données accessibles à votre compte.',
  'Ask about your data… type / for commands': 'Interrogez vos données… tapez / pour les commandes',
...
const ja: Dictionary = {
  ...
  'Read the documentation article "{title}" (id: {id}) and use it to answer.':
    'ドキュメント記事「{title}」（id: {id}）を読んで、その内容をもとに回答してください。',
  'Open in knowledge base': 'ナレッジベースで開く',
  'Remove from context': 'コンテキストから外す',
  'How can I help you today?': '何かお手伝いしましょうか？',
  'Ask about the data available to your account.': 'アカウントで利用できるデータについて質問できます。',
  'Ask about your data… type / for commands': 'データについて質問… / でコマンド',
...
const ko: Dictionary = {
  ...
  'Read the documentation article "{title}" (id: {id}) and use it to answer.':
    '"{title}" 문서(id: {id})를 읽고 그 내용을 바탕으로 답변해 주세요.',
  'Open in knowledge base': '지식 베이스에서 열기',
  'Remove from context': '컨텍스트에서 제거',
  'How can I help you today?': '무엇을 도와드릴까요?',
  'Ask about the data available to your account.': '계정에서 사용할 수 있는 데이터에 대해 질문하세요.',
  'Ask about your data… type / for commands': '데이터에 대해 질문… 명령은 / 입력',
...
const th: Dictionary = {
  ...
  'Read the documentation article "{title}" (id: {id}) and use it to answer.':
    'อ่านบทความคู่มือ "{title}" (id: {id}) แล้วใช้ข้อมูลนั้นในการตอบ',
  'Open in knowledge base': 'เปิดในฐานความรู้',
  'Remove from context': 'นำออกจากบริบท',
  'How can I help you today?': 'ให้ช่วยอะไรดีครับ/คะ?',
  'Ask about the data available to your account.': 'สอบถามข้อมูลที่บัญชีของคุณเข้าถึงได้',
  'Ask about your data… type / for commands': 'ถามเกี่ยวกับข้อมูลของคุณ… พิมพ์ / เพื่อดูคำสั่ง',
...
const vn: Dictionary = {
  ...
  'Read the documentation article "{title}" (id: {id}) and use it to answer.':
    'Đọc bài viết tài liệu "{title}" (id: {id}) và dùng nó để trả lời.',
  'Open in knowledge base': 'Mở trong cơ sở tri thức',
  'Remove from context': 'Bỏ khỏi ngữ cảnh',
  'How can I help you today?': 'Tôi có thể giúp gì cho bạn?',
  'Ask about the data available to your account.': 'Hỏi về dữ liệu mà tài khoản của bạn truy cập được.',
  'Ask about your data… type / for commands': 'Hỏi về dữ liệu của bạn… gõ / để xem lệnh',
...
const zh: Dictionary = {
  ...
  'Read the documentation article "{title}" (id: {id}) and use it to answer.':
    '阅读文档文章“{title}”（id: {id}）并据此回答。',
  'Open in knowledge base': '在知识库中打开',
  'Remove from context': '从上下文中移除',
  'How can I help you today?': '有什么可以帮您？',
  'Ask about the data available to your account.': '查询您的账号可访问的数据。',
  'Ask about your data… type / for commands': '查询您的数据… 输入 / 查看命令',
  'Send message': '发送消息',
