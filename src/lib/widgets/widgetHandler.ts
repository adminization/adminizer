...
export class WidgetHandler {
    ...
    private async getBuiltInFilterWidgets(user: User, i18n: I18n): Promise<WidgetConfig[]> {
        ...
        for (const modelName of modelNames) {
            ...
            if (!await this.adminizer.accessRightsHelper.checkPermission(readToken, user)) {
                continue;
            }
        ...
        if (this.widgets.length) {
            ...
            for (const widget of this.widgets) {
                if (widget.widgetType === 'switcher') {
                    if (await this.adminizer.accessRightsHelper.checkPermission(this.resolveAccessRightsToken(widget), dashboardUser)) {
                        widgets.push({
