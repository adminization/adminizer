...
* `AiAssistantToggle` renders the sparkles button in the header. The button displays a spinner while a request is in flight and disables itself if no models are accessible.
* `AiAssistantPanel` slides in from the right, occupying roughly a quarter of the viewport so the underlying page remains visible. It exposes model selection, the conversation transcript, and the prompt form.
