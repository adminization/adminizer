...
* `AiAssistantToggle` renders the sparkles button in the header. When pressed, it opens a docked chat panel that occupies the rightmost quarter of the viewport.
* `AiAssistantPanel` hosts the chat experience, including model selection, streaming conversation history, and the composer form. The main application content shifts left while the panel is visible so both areas remain accessible.
