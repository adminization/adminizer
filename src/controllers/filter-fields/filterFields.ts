 ...
 * Excludes: system fields, complex JSON fields, modelFilters.excludeFromFilters
...
export default async function filterFields(req: ReqType, res: ResType) {
    ...
    const filterConfig = req.adminizer.config.modelFilters?.[entity.name];
    ...
    for (const [fieldName, field] of Object.entries(fields)) {
        ...
        const customFilterHandler = customFilterHandlerId
            ? req.adminizer.filterCustomFieldHandler?.get(customFilterHandlerId)
            : undefined;
    ...
    // Apply whitelist if specified (for models where we want explicit control)
    if (filterConfig?.includeInFilters?.length) {
        return res.json({
            model: entity.name,
            fields: filterFields.filter(f => filterConfig.includeInFilters!.includes(f.id)),
        });
    }
