...
interface ValidationContext {
    ...
    totalConditionsExceeded: boolean;
}
    ...
    private validateCondition(
        ...
        if (context.totalConditions > FILTER_SECURITY_LIMITS.MAX_TOTAL_CONDITIONS) {
            ...
            this.logSecurityEvent(
                'TOO_MANY_TOTAL_CONDITIONS',
                condition.id,
                `Attempted ${context.totalConditions} total conditions, max ${FILTER_SECURITY_LIMITS.MAX_TOTAL_CONDITIONS}`,
                context.totalConditions
            );
            errors.push({
