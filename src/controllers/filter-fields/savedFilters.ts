 ...
 * Returns translations for filter UI
 */
export async function getFilterLocales(req: ReqType, res: ResType) {
    const messages = {
        "Filters": "",
        "Saved filters": "",
        "Active filter": "",
...
export async function applyTemporaryFilter(req: ReqType, res: ResType) {
    ...
    if (!conditions || !Array.isArray(conditions)) {
        return res.status(400).send({ error: 'Conditions are required' });
    }
...
export async function saveFilter(req: ReqType, res: ResType) {
    ...
    if (!name) {
        return res.status(400).send({ error: 'Filter name is required' });
    }

    if (!conditions || !Array.isArray(conditions)) {
        return res.status(400).send({ error: 'Filter conditions are required' });
    }
    ...
    try {
        ...
        if (existingFilter && !filterId) {
            ...
            return res.status(409).json({
                error: 'Filter with this name already exists',
...
export async function deleteFilter(req: ReqType, res: ResType) {
    ...
    try {
        ...
        // Ignore "not found" error after successful deletion (Waterline quirk)
        if (error.message && error.message.includes('not found')) {
            Adminizer.log.debug(`Filter ${filterId} already deleted or not found`);
            return res.json({
                success: true
            });
        }
