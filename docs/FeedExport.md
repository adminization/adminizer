...
- **apiKey** — это единственный секретный ключ. Любой, кто знает URL, может получить данные
- **Не используйте** для конфиденциальных данных без дополнительной защиты
- При компрометации ключа — перегенерируйте его в UI фильтра
- API не проверяет права доступа — данные отдаются всем, кто знает ключ
