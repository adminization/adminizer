...
describe('Example Model Filters - Sequelize Integration', () => {
    ...
    describe('11. ConditionValidator security limits', () => {
        it('should reject filters when total conditions exceed max limit', () => {
            const validator = new ConditionValidator({
                title: { type: 'string' }
            });

            const conditions: FilterCondition[] = Array.from({ length: 501 }, (_, index) => ({
                id: `test-total-${index}`,
                field: 'title',
                operator: 'eq',
                value: `value-${index}`
            ...
            expect(result.valid).toBe(false);
            expect(result.errors[0]?.code).toBe('TOO_MANY_TOTAL_CONDITIONS');
            expect(result.errors[0]?.message).toBe('Too many total conditions (max 500)');
        });
    });
});
