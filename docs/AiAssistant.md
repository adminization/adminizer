...
The demo fixture ships with the assistant enabled so it can be exercised immediately after compiling the backend.

* The fixture configuration (`fixture/adminizerConfig.ts`) sets `aiAssistant.enabled` to `true`, keeps the `dummy` model as the default, and exposes the sparkles toggle in the header.
* Run `npm run build` before starting the fixture so that the freshly compiled controller and handler code is available inside `dist/`. The build step wires the assistant routes into the Express app that the fixture boots.
